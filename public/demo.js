...
const isValid = (str) => {
  ...
  if (!error_data.includes(str)) return true;
  else return false;
};
    ...
    switch (socketIdentity) {
      case reqGBValues.hsm:
        if (this.retryCount < 5) {
          let tokenValue = isValid($("#token_id").val())
            ? $("#token_id").val()
            : "0";
      ...
      case reqGBValues.hsi:
        if (this.hsiRetryCount < 5) {
          ...
          if (
            ...
            console.log(
              "%c[Socket]: Invalid Token !",
              "background: red; color: white"
            );
            return;
          }
          connectHsi(tokenValue, sidValue, datacenter)
            .then((msg) => {
              console.log("hsi connected", msg);
            })
            .catch((err) =>
              console.log(
                "%c[Socket]: Failed to connect ! " + err,
                "background: red; color: white"
              )
            );
        } else {
...
function connectHsm(token, sid) {
  return new Promise((resolve, reject) => {
    ...
    userWS.onclose = function () {
      ...
      if (retrySocket.hsmRetry) retrySocket.initiateRetry("Hsm");
      reject("closed");
    };
...
resumeandpause = function (typeRequest, channel_number) {
  ...
  jObj["channelnums"] = channel_number.split(",").map(function (val) {
    return parseInt(val, 10);
  });
  if (userWS != null) {
    let req = JSON.stringify(jObj);
    userWS.send(req);
    if (typeRequest == "cr") retrySocket.hsmRetry = true;
  }
  if (typeRequest == "cp") {
...
function connectHsi(token, sid, datacenter) {
  return new Promise((resolve, reject) => {
    ...
    if (datacenter == "adc") url = "wss://cis.kotaksecurities.com/realtime";
    ...
    hsWs.onclose = function () {
      ...
      if (retrySocket.hsiRetry) retrySocket.initiateRetry(reqGBValues.hsi);
      reject("closed");
    };
